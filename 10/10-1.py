def calculate_scores(file_path):
    ...
    def is_valid_move(curr, next_pos):
        """Check if moving from curr to next_pos is valid."""
        x, y = next_pos
        return (
    ...
    def bfs(start):
        """Do BFS to find all reachable trailends from a trailhead."""
        queue = [start]
        visited = set()
        reachable_nines = set()
        ...
        while queue:
            ...
            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                next_pos = (x + dx, y + dy)
                if next_pos not in visited and is_valid_move((x, y), next_pos):
